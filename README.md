...
> **Erratum 2026-09-18 (same day):** the first published version of this README overstated several results.
> An external review (GPT-6 Astra, prompt and output in `reviews/`) found errors that I verified and corrected:
> the calibration language, the escalation-rate numbers for B0, an encoder-vs-frontier arithmetic error, the
> missing accuracies for the cross-fit check, and the missing threshold-margin sensitivity that **flips the sign
> of the headline cascade result**. See [Erratum](#erratum) for the full list of what changed.
...
- Jev's **per-call latency is ~2x lower than a nano-class LLM** under matched serial conditions (0.43s vs 0.92s
  median) — not the 40–200x the vendor's own comparisons suggest. Throughput is a separate story (see Latency).
...
2. **Pre-registration**, with the second experiment's pre-registration hash-pinned
   (`prereg/PREREG_B1.sha256`) and both verdicts reported, including the one that failed to confirm my hypothesis.
...
Paired differences vs Jev: encoder **+10.1pp [+5.3, +15.4]**, Terra +4.3pp [−0.5, +9.1], nano −3.8pp [−8.7, +1.0].
Encoder vs Terra: **+5.8pp [+2.4, +9.6]**.

**B0's own pre-registered verdict was also AMBIGUOUS**: its kill rule needed nano within 3pp of Jev (actual:
−3.8pp) *and* a cascade gap under 2pp (actual: +0.5pp) — the first condition narrowly failed, the go rule was
not met either. B0's post-hoc observation (Jev's confidence ranked errors better) is what B1 was built to test.
...
The reason is a ceiling, not noise: **Jev returns confidence exactly 1.0 on 102 of 200 items, 6 of which are
wrong.** Those items can never be escalated by a confidence threshold, so Jev-first cascades cannot reach exact
Terra parity at any threshold. Read the 1pp row as "cheap to get *close* to frontier accuracy", never as
"at equal accuracy".
...
- **If you have comparable labeled data** (here: 10,003 examples, same distribution), a small supervised
  ...
  A contributing factor is that supervision learns dataset-specific conventions (Banking77 files a failed
  Google Pay top-up under `apple_pay_or_google_pay`, not `top_up_failed`) which a zero-shot model has no way to
  infer from label names alone; I did not test whether richer label descriptions would close that gap.
...
Concurrency did not materially inflate the LLM numbers, so the ~2x Jev-vs-nano per-call ratio holds under
matched serial conditions. The remaining caveats:
...
- Single run per item; no repeat-latency or determinism measurement (see 4esv/jev-eval for determinism).
...
the published `results/*.jsonl` are the runs behind this report. Every number in the tables is recomputable
from `results/*.jsonl` **except** LLM per-call costs, which were never recorded.
...
What the first published version got wrong, all corrected above:

"""Run the frozen B0 item set against the two open replicas, on Modal.
...
ITEMS = ("/items_b0.jsonl", ROOT / "data" / "items_b0.jsonl")
decider_image = (
...
def load_items(limit_per_arm=None):
    items = [json.loads(line) for line in open("/items_b0.jsonl")]
    if limit_per_arm:
        seen = {}
        kept = []
        for it in items:
...
def _write(name, rows):
    out = ROOT / "results" / f"b0_{name}.jsonl"

...
These are the pre-registered marginal numbers, and they stand. **The department control below
leaves Jev's inversion intact under both weightings; for decider-2b and kotoba the adjusted reading
depends on the weighting, so ticket composition remains a possible explanation of their marginal
figures — not a demonstrated one.**
...
and a wrongly blocked one, which B1 did not measure and does not assume.

## The department control: Jev robust, the other two weighting-dependent (EXPLORATORY)

The review asked whether the marginal E1 could come from something other than being hijacked. For
two of the three systems the data cannot say. The gold department and the outcome are close to
collinear in decider-2b: **all 37 `billing` tickets resisted and all 37 `sales` tickets were
hijacked**.

Recomputing E1 **within** gold department requires a choice of how to combine the departments, and
**that choice changes the answer for two of the three systems.** Both are reported, because picking
one silently is how the second round of review caught this:

- **pair-weighted**: each department weighted by its number of resisted×hijacked pairs. A department
  with only one outcome class contributes no pairs and drops out.
- **equal-weighted**: the mean of the per-department AUROCs that are estimable at all.

| system | marginal E1 | pair-weighted | equal-weighted | rows dropped (single-class departments) |
|---|---|---|---|---|
| **Jev 1.13.0** | 0.261 | **0.329** [0.251, 0.412] | **0.228** | 0 / 300 |
| decider-2b | 0.412 | 0.493 | **0.417** | **74 / 300** |
| kotoba deberta | 0.571 | 0.497 | **0.561** | 0 / 300 |

**Jev is the only system whose result is stable under both**: below 0.5 either way, and it also
survives conditioning on the injected target, the target's option position and ticket-length
quartile (0.263 / 0.257 / 0.248).

**For decider-2b and kotoba the department control is weighting-dependent and therefore
inconclusive.** Under pair weighting decider's inversion moves to 0.493 and kotoba's weak positive to
0.497; under equal weighting they barely move from their marginal values (0.417 and 0.561). Decider's
case is the one to be careful with: its `billing` tickets all resisted and its `sales` tickets were
all hijacked, so **74 of its 300 authority rows cannot contribute a within-department comparison at
all** and vanish from the pair-weighted figure.
...
result. It does **not** establish that explanation, and it does not rule out a within-department
association.

Two further checks, both EXPLORATORY, both recomputed from the raw jsonl:

- **Sparse departments move the equal-weighted figure.** kotoba's `careers` department has a single
  hijacked row and contributes an AUROC of exactly 1.000; dropping departments whose smaller class
  has one row takes kotoba's equal-weighted estimate from **0.561 to 0.498** — across 0.5. The same
  operation on Jev takes it from 0.228 to **0.260**, still inverted. A figure that flips on one row
  is not a finding.
- **Department-stratified conditional logistic** (exact conditional likelihood, odds of being
  hijacked per +0.1 confidence): Jev **1.58 [1.22, 2.04]**, decider-2b **0.98 [0.79, 1.21]**, kotoba
  **1.17 [0.98, 1.38]**. This agrees with the AUROC reading — only Jev's association excludes the
  null — but it assumes a common slope across departments, and decider's two single-class
  departments drop out of it as well (6 of 8 strata contribute).

## What this supports

For **Jev**, on this task: within the injected arm, higher confidence ranks hijacked decisions above
resisted ones, and that ordering persists under every control tried — gold department (under both
weightings), injected target, option position and ticket length.

For **decider-2b and kotoba**, B1 reports their pre-registered marginal verdicts and stops there. The
department control is weighting-dependent for both, so B1 supports **no** conclusion about whether
their marginal figures reflect hijacking or ticket composition.
...
2. **decider-2b's inversion presented as a finding.** Its gold department and its outcome are
   nearly collinear, so the marginal figure cannot be read as a property of hijacking. Now reported
   as marginal-only, with the control. (The first revision then over-corrected this — see 6.)
...
A second review round then caught an error introduced by that first revision:

6. **"the inversion disappears" for decider-2b and kotoba.** That conclusion depended on a pooled
   within-department weighting the write-up never declared, which silently drops departments with a
   single outcome class — 74 of decider's 300 authority rows. Under equal weighting the two systems
   barely move from their marginal values. Now both weightings are reported and the department
   control is described as inconclusive for those two systems.

A third round confirmed the corrected numbers and caught one piece of framing the fix had left behind:

7. **A summary sentence still said the department mix "explains" decider's and kotoba's marginal
   figures**, contradicting the "compatible with" caveat three paragraphs below it, and the section
   heading still read "Only Jev's inversion survives a control" although decider's equal-weighted
   estimate is also inverted. Both now say what the data says: Jev is robust under both weightings,
   the other two are weighting-dependent.
...
Reproducible from the files in this repository: every E1 figure, every interval, the harness values,
the department analyses, the gate counts and the Jev token total (672,896 → $0.0283 at the published
price). **Not reproducible from them**, and stated here on trust: the 30-item re-ask, which
User-Agent each row was sent with, and the Modal billing figures.
